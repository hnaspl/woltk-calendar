  ...
  <div class="space-y-6">
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-border-default">
            <tr v-for="role in displayedRoles" :key="role.id" class="hover:bg-bg-tertiary/50 transition-colors">
              ...
              <td class="px-4 py-2.5">
                <span class="text-accent-gold text-xs font-medium">{{ role.permissions.length }} {{ t('roles.permissionsCount') }}</span>
              </td>
    ...
    <WowCard v-if="canManageRoles">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-bg-tertiary border-b border-border-default">
              <th class="text-left px-4 py-2.5 text-xs text-text-muted uppercase">{{ t('roles.granter') }}</th>
              <th class="text-center px-4 py-2.5 text-xs text-text-muted uppercase"></th>
              <th class="text-left px-4 py-2.5 text-xs text-text-muted uppercase">{{ t('roles.grantee') }}</th>
    ...
    <WowModal v-model="showRoleModal" :title="editingRole ? t('roles.editRole') : t('roles.createRole')" size="lg">
      <form @submit.prevent="saveRole" class="space-y-4">
        ...
        <div>
          <label class="block text-xs text-text-muted mb-1">{{ t('roles.levelLabel') }}</label>
          <input
    ...
    <WowModal v-model="showDeleteConfirm" :title="t('roles.deleteRole')" size="sm">
      <p class="text-text-muted">
        {{ t('roles.deleteConfirm', { role: deleteTarget?.display_name }) }}
      </p>
      <template #footer>
        <div class="flex justify-end gap-3">
...
async function loadPermissions() {
  ...
  try {
    ...
    toast.error(err?.response?.data?.message ?? t('roles.toasts.failedToLoadPermissions'))
  } finally {
...
async function loadGrantRules() {
  ...
  try {
    ...
    toast.error(err?.response?.data?.message ?? t('roles.toasts.failedToLoadRules'))
  } finally {
...
async function saveRole() {
  ...
  try {
    ...
    if (editingRole.value) {
      ...
      toast.success(t('roles.toasts.roleCreated'))
    }
...
async function doDeleteRole() {
  ...
  try {
    ...
    toast.success(t('roles.toasts.roleDeleted'))
    showDeleteConfirm.value = false
    await loadRoles()
  } catch (err) {
    toast.error(err?.response?.data?.message ?? t('roles.toasts.failedToDeleteRole'))
  } finally {
...
async function doCreateGrantRule() {
  ...
  try {
    ...
    toast.success(t('roles.toasts.grantRuleAdded'))
...
async function doDeleteGrantRule(rule) {
  try {
    ...
    toast.success(t('roles.toasts.grantRuleRemoved'))
    await Promise.all([loadRoles(), loadGrantRules()])
  } catch (err) {
    toast.error(err?.response?.data?.message ?? t('roles.toasts.failedToRemoveRule'))
  }
}
</script>
